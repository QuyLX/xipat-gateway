...
export class refactDb1649221187632 implements MigrationInterface {
    name = 'refactDb1649221187632'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`username\` varchar(255) NOT NULL, \`role\` enum ('user', 'admin', 'partner') NOT NULL DEFAULT 'user', \`plan\` enum ('basic', 'pro', 'enterprise') NOT NULL DEFAULT 'basic', \`apikey\` varchar(255) NOT NULL, \`clientid\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`monitor\` (\`id\` int NOT NULL AUTO_INCREMENT, \`apikey\` varchar(255) NOT NULL, \`requestSuccess\` int NOT NULL DEFAULT '0', \`requestTotal\` int NOT NULL DEFAULT '0', \`time\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    }
